...
const char* SERVER_URL    = "https://compostanalyzer.vercel.app/data";
const char* COMMAND_URL   = "https://compostanalyzer.vercel.app/command";
...
// Feature priority: 1. Moisture  2. Temperature  3. Methane (least important)
#define MOISTURE_WET_MAX  1500      // <= TOO_WET
#define MOISTURE_DRY_MIN  2000      // >= TOO_DRY
#define TEMP_THRESH       29.09f    // middle zone: > TOO_DRY, <= COMPOST_READY
...
// 2. Temperature SECONDARY — middle zone only (~3-5C difference)
// 3. Methane LEAST IMPORTANT — not used
String classify(float temp, int moisture, int methane) {
  if (moisture <= MOISTURE_WET_MAX)  return "TOO_WET";
  if (moisture >= MOISTURE_DRY_MIN)  return "TOO_DRY";
  // middle zone (1500–2000): use temperature
  if (temp > TEMP_THRESH)            return "TOO_DRY";
  return "COMPOST_READY";
}
